...
<div style="padding:4px 5px 0px 5px;">
	...
	<%-- <c:if test="${role==1}"> --%>
		...
		<table id="List" style="height:auto;border:1px solid #ccc;">
			<thead>
				<tr>
					<th data-options="field:'mid',width: 50,hidden:true">编号ID</th>
					<th data-options="field:'principalname',width: 50"formatter="myFormatter">名称</th>
					<th data-options="field:'departmentname',width: 15" >部门</th>
					<th data-options="field:'leadership',width: 15 " >分管领导</th>
				</tr>
			</thead>
		</table>
	</div>
	...
	$(function(){ 
      ...
      /*  console.log(row) */
       ...
       ShowInfo('详情','../month/monthplanauditparticularPage?mid='+row.mid);
			...
			$("#modalwindowContent").window({
				...
				iconCls : 'fa fa-list'
			     ...
			     var did="${did}";
			        ...
			        setTimeout("location.href='monthplanaudit'", 500);
			    }
	  ...
	  function myFormatter(value, row, index){
		var principalname=row.years+'年'+row.months+'月'+'"'+row.name+'"'+'工作计划';
