...
public class YearPlanController {
	...
	//年计划详情页更新跳转
		...
		public ModelAndView yearplannextskip(@Param("yid")int yid,HttpServletRequest request){
			...
			if(user.getRid()==1){
				/*
				 * 建立group分组部门分别为哪些部门，replace储存个人信息，
				 */
	...
	public ModelAndView yearplanparticulars(@Param("yid")int yid,HttpServletRequest request){
		...
		List<Attachments> attachments=attachmentsService.display(new Attachments(4,yid));
		if(attachments!=null)
		mv.addObject("files",attachments);
	...
	if(a!=null){
		...
		if(yearplan.getFiles().length!=0){
			List<Attachments> attachments=new ArrayList<Attachments>();
			for(int i=0;i<yearplan.getFiles().length;i++)attachments.add(new Attachments(4,a,yearplan.getFiles()[i]));
			//上传附件
	...
	//删除条目
		...
		//添加年计划
				...
				public Json updateyearplan(@RequestBody List<YearPlanNext> yearPlannext ,HttpServletRequest request){
					try{
						Integer a=yearPlanNextService.creatyearplannext(yearPlannext);
						if(a!=null){
							if(yearPlannext.get(0).getFiles().length!=0){
								List<Attachments> attachments=new ArrayList<Attachments>();
								for(int i=0;i<yearPlannext.get(0).getFiles().length;i++)attachments.add(new Attachments(4,yearPlannext.get(0).getYid(),yearPlannext.get(0).getFiles()[i]));
								//上传附件
								attachmentsService.creates(attachments);
							}
							return new Json(200,"更改成功",null);
