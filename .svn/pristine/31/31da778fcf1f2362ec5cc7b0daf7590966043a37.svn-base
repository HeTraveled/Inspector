			...
			<div class="cc">
					...
					<table
							...
							<tbody>
								<tr>
									...
									<th align="center" width="18%">发布人</th>
								...
								<c:forEach items="${annc}" begin="0" end="4" var="a">
									...
									<tr style="height:22px;"
										...
										<td
											...
											href="javascript:Show2('${a.title }','${a.id}')">
												 <fmt:formatDate
													value="${a.date }" pattern="yyyy-MM-dd" /> </a>
										</td>
										...
										<td style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis;
											-o-text-overflow:ellipsis;-icab-text-overflow: ellipsis;-khtml-text-overflow: ellipsis;
											-moz-text-overflow: ellipsis;-webkit-text-overflow: ellipsis;">   
											        <c:if test="${a.type==1}">协同文件 <c:if test="${uid==a.uid}">(发)</c:if>
											        <c:if test="${uid!=a.uid}">(收)</c:if> </c:if>

													<c:if test="${a.type==2}">办理文件<c:if test="${uid==a.uid}">(发)</c:if>
											        <c:if test="${uid!=a.uid}">(收)</c:if></c:if>
													<c:if test="${a.type==3}">集团红头<c:if test="${uid==a.uid}">(发)</c:if>
											        <c:if test="${uid!=a.uid}">(收)</c:if></c:if>
										</td>
									</tr>
				...
				<div id="notePart" class="easyui-draggable"
					...
					<div id="title7" class="panel-header" style="border: 0">
						<div class="panel-title" style="">
							<span class="fa fa-calendar-minus-o"></span>&nbsp;&nbsp;新闻简讯 <span
								class="aaa">
								<a style="float: right;"
								href="javascript:parent.addTab('新闻简讯一览表','../news/newslistskip','fa fa-file-text-o')">更多&nbsp;&nbsp;</a>
							</span>
						</div>
						<div class="panel-tool">
							...
							<th align="left" width="40%" >
								...
								<c:forEach items="${news}" var="n">
									<tr style="height:25px;"
										...
										<td align="left">
											<a class="color-black"
												href="javascript:newsskip('${n.body}')">${n.title}</a>
										</td>
									</tr>
								</c:forEach>
								</c:if>
