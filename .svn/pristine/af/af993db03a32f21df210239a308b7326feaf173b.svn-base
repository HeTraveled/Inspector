...
public class Task {
		...
		try {
			...
			if (inspectors != null && inspectors.size() != 0) {
				...
				for (int i = 0; i < inspectors.size(); i++) {
					...
					// 推送微信消息
					List<User> res = userService.selectdid(inspector
							.getResponsibility());
					String assistDept = inspector.getAssistDept();
					List<Integer> id = new ArrayList<Integer>();
					// 协办领导
					if (inspector.getLeadership() != null
							&& inspector.getLeadership() != 0)
						id.add(inspector.getLeadership());
					// 一人负责管理多部门
					...
					if (res != null && res.size() != 0)
						for (int z = 0; z < res.size(); z++)
							if (pri.getUid() != res.get(z).getUid())
								id.add(res.get(z).getUid());
					// 负责人计数
					if (deptService.countPri(pri.getUid()) >= 1)
						if (pri.getDid() != inspector.getResponsibility())
							id.add(pri.getUid());
					// 协助单位
					...
					if (id != null && id.size() != 0) {
						for (int q = 0; q < id.size(); q++) {
							...
							if (user.getWechat() != null
									&& !user.getWechat().equals("")) {
								WechatController.sendIns(
										inspector.getSource(),
										HTMLSpirit.delHTMLTag(
												inspector.getBody())
												.replaceAll("&nbsp;", " "),
										"已超期", "工作项已超期，请抓紧时间处理以上工作", user
												.getWechat().trim(), inspector
												.getIid());
							}
						}
					}
					iid.add(inspector.getIid());
					scores.add(new Score(deptService.selectByPrimaryKey(
							inspector.getResponsibility()).getPrincipal(), 2,
							inspector.getIid(), 0, 1, 1, date.getMonth() + 1,
							Integer.parseInt(date.toLocaleString().substring(0,
									4))));
					// 查询前一天是否进行反馈 未反馈则扣分
					...
					if (inspectorProgressService.selectDay(sdf.format(time),
						...
						scoreService.create(new Score(deptService
								.selectByPrimaryKey(
										inspector.getResponsibility())
								.getPrincipal(), 1, inspector.getIid(), 0, 1,
								1, date.getMonth() + 1, Integer.parseInt(date
										.toLocaleString().substring(0, 4))));
					}
					;
				}
				// 将数据批量修改为超期
			...
			if (feedbackDay != null && feedbackDay.size() != 0) {
				for (int i = 0; i < feedbackDay.size(); i++) {
					...
					if (inspectorProgressService.selectDay(sdf.format(time),
						...
						scoreService.create(new Score(deptService
								.selectByPrimaryKey(
										feedbackDay.get(i).getResponsibility())
								.getPrincipal(), 1,
								feedbackDay.get(i).getIid(), 0, 1, 1, date
										.getMonth() + 1, Integer.parseInt(date
										.toLocaleString().substring(0, 4))));
					}
					;
				}
	...
	public void feedbackWeeks() {
		...
		if (inspectors != null && inspectors.size() != 0) {
			for (int i = 0; i < inspectors.size(); i++) {
				...
				if (inspectorProgressService.selectDay(sdf.format(time),
					...
					scoreService.create(new Score(deptService
							.selectByPrimaryKey(
									inspectors.get(i).getResponsibility())
							.getPrincipal(), 1, inspectors.get(i).getIid(), 0,
							1, 1, date.getMonth() + 1, Integer.parseInt(date
									.toLocaleString().substring(0, 4))));
				}
				;
			}
		...
		// List<WeeksPlan> weeksPlans=weeksPlanService.selectEme(year, month,
		// week);
		...
		// emergencies.add(new
		// Emergency(did,4,2,weeksPlan.getWeeksPlanNext().get(j).getId()));
		// }
		// }
		// }
	...
	public void weeksplanstate() {
		List<WeeksPlanNext> weeksplannext = weeksplannextService
				.weeksplannextoverdue();
		if (weeksplannext != null && weeksplannext.size() != 0) {
			List<Integer> id = new ArrayList<Integer>();
			for (int i = 0; i < weeksplannext.size(); i++)
				id.add(weeksplannext.get(i).getId());
	...
	public void monthplanstate() {
		List<MonthPlanNext> monthPlanNext = monthplannextService
				.monthplannextoverdue();
		if (monthPlanNext != null && monthPlanNext.size() != 0) {
			List<Integer> id = new ArrayList<Integer>();
			for (int i = 0; i < monthPlanNext.size(); i++)
				id.add(monthPlanNext.get(i).getId());
	...
	public void yearplanstate() {
		List<YearPlanNext> yearPlanNext = yearPlanNextService
				.yearplannextoverdue();
		...
		try {
			...
			if (schedules != null && schedules.size() != 0) {
				...
				for (int i = 0; i < schedules.size(); i++) {
					...
					if (schedule.getScheduleReminds() != null
							&& schedule.getScheduleReminds().size() != 0) {
						for (int j = 0; j < schedule.getScheduleReminds()
								.size(); j++) {
							ScheduleRemind sr = schedule.getScheduleReminds()
									.get(j);
							if (sr.getUid() == 0)
								continue;
							if (sr.getWechat() != null
									&& !sr.getWechat().equals("")) {
								wc.sendtemplate(4, schedule.getBody(), schedule
